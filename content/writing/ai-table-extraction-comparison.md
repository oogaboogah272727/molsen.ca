...
When extracting tables from technical documents, the difference between 11% and 100% accuracy is not just a statistical curiosity—it is the difference between a usable system and one that generates more work than it saves. After running identical documents through three extraction approaches—Docling, ScaleDP, and Claude-based sub-agents—the results challenge common assumptions about where open-source tools excel and where AI-native approaches have made decisive gains.
...
For any downstream use of extracted tables—whether for cost analysis, proposal comparison, or knowledge base population—the table title is essential context. A table of numbers labeled "Project Costs" serves a different analytical purpose than one labeled "Historical Spending." Without accurate title detection, extracted data requires expensive manual review before it becomes useful.

Here is where the tools diverged dramatically.

Docling detected titles correctly on only 11% to 14% of tables across the test documents. ScaleDP improved to roughly 20%, but with a significant caveat: many of its "correct" titles were actually wrong. In the BHP proposal, ScaleDP labeled Table 1 with the content "+1 604 681 4196 office"—the phone number from the letterhead, not the actual table title "Summary of Project Costs by Task."

The Claude-based approach achieved 100% title detection, correctly identifying "Table 1: Summary of Project Costs by Task" and "Detailed Budget Breakdown by Task and Staff" with complete accuracy.

---

## The 44% False Positive Problem

Beyond title accuracy, both open-source tools generated substantial noise. Docling detected 27 tables in the BHP proposal, of which 12 were completely empty (0x0 dimensions). This represents a 44% false positive rate—meaning nearly half of the "tables" found were phantoms that would pollute any downstream processing.
...
For anyone planning to process documents at scale, this matters enormously. A 44% false positive rate means your pipeline spends nearly half its effort on garbage. Every false table requires storage, processing, and eventual filtering—resources that add up quickly across thousands or millions of documents.
...
For small-scale extraction—tens of documents where manual review is feasible—any tool will work, provided you budget time for cleanup. ScaleDP's speed makes it attractive for quick extraction tasks where you will verify results anyway.

For medium-scale processing—hundreds to thousands of documents—the false positive rate becomes the dominant concern. At 44% false positives, you are effectively doubling your data volume with noise. The time saved on extraction gets consumed by filtering and validation.
...
For raw extraction speed at massive scale with tolerance for post-processing, ScaleDP offers throughput—but budget for significant cleanup. Column alignment issues and false positives will require additional processing steps.

For structural accuracy on simpler tables, both open-source tools perform adequately, though the false positive rate remains a concern.

The uncomfortable conclusion is that document understanding still requires understanding. Tools that attempt pure pattern matching against PDF structure will continue to find patterns where none exist—letterheads as tables, phone numbers as titles, resume headers as data structures. Tools that bring semantic comprehension to extraction will continue to outperform on the metrics that matter for downstream use.
